...
- **Promoted-widget evidence limit**: the existing real-host check changes an ordinary node widget and constructs source fields from that node. Its historical pass does not establish a real promoted-subgraph binding, propagation into the inner widget, or correct prompt serialization. Those interactions require dedicated qualification.
